...
export default function Dashboard() {
  ...
  const loadData = async () => {
    try {
      const [userData, subjectsData, progressSummary] = await Promise.all([
        api.getMe(),
        api.getSubjects(),
  ...
  const stats = [
    { icon: BookOpen, label: 'Courses', value: subjects.length, color: 'from-blue-500 to-cyan-500' },
    {
      ...
      value: summary?.overall?.topics_learned ?? '—',
      color: 'from-purple-500 to-pink-500',
    },
    {
      icon: Clock,
      label: 'Study Hours',
      value: summary?.hours ? `${summary.hours.total_hours}h` : '—',
      color: 'from-amber-500 to-orange-500',
    },
    {
      icon: Trophy,
      label: 'Quizzes Passed',
      value: summary?.quizzes?.count ? `${summary.quizzes.passed}/${summary.quizzes.count}` : '—',
