      ...
      <div className="h-full glass border-r border-white/10 flex flex-col">
        ...
        <div className="h-16 flex items-center px-4 border-b border-white/10">
          <div className="flex items-center gap-3 min-w-0">
            <img src="/logo.web.png" alt="Academia AI" className="h-9 w-auto object-contain flex-shrink-0" />
            {!collapsed && (
        ...
        <div className="border-t border-white/10 p-3">
          ...
          >
            {user?.imageUrl ? (
              <img src={user.imageUrl} alt="" className="w-6 h-6 rounded-lg object-cover flex-shrink-0" />
            ) : (
              <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center flex-shrink-0">
                <span className="text-[10px] font-bold text-white">{initials}</span>
              </div>
            )}
            {!collapsed && (
              <div className="min-w-0 text-left">
                <span className="text-sm truncate block">{fullName}</span>
                <span className="text-[10px] text-white/40 truncate block">{(user?.publicMetadata as any)?.domain || 'Learner'}</span>
              </div>
            )}
          </button>
        </div>
